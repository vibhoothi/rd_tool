...
class RDWork(Work):
    ...
    def parse(self, stdout, stderr):
        ...
        split = None
        try:
            split = self.raw.decode('utf-8').replace(')',
                                                     ' ').split(maxsplit=56)
            self.pixels = split[1]
            self.size = split[2]
            self.metric = {}
            ...
            for metric_name in ['psnr_y', 'psnr_cb', 'psnr_cr', 'ciede2000', 'float_ssim',
                                'float_ms_ssim', 'psnr_hvs_y', 'psnr_hvs_cb', 'psnr_hvs_cr', 'psnr_hvs', 'cambi']:
                self.metric['vmaf_' +
                            metric_name] = root.find("pooled_metrics/metric[@name='" +
                                                     metric_name +
                                                     "']").get('mean')
            self.metric['vmaf'] = root.find(
                "pooled_metrics/metric[@name='vmaf']").get('mean')
            self.metric['vmaf_neg'] = root.find(
                "pooled_metrics/metric[@name='vmaf_neg']").get('mean')
            for metric_name in ['apsnr_y', 'apsnr_cb', 'apsnr_cr']:
                self.metric['vmaf_' +
                            metric_name] = root.find("aggregate_metrics").get(metric_name)
            self.failed = False
        except (IndexError, ET.ParseError):
            rd_print(self.log, 'Decoding result for ' + self.filename +
                     ' at quality ' + str(self.quality) + ' failed!')
    ...
    def execute(self):
        try:
            ...
            daalatool_dir = os.getenv(
                "DAALATOOL_DIR", os.path.join(
                    slot.machine.work_root, "daalatool"))
            ...
            if self.no_delete:
                command += 'NO_DELETE=1 '
            command += slot.work_root + \
                '/rd_tool/metrics_gather.sh ' + shellquote(input_path)
            slot.start_shell(command)
            (stdout, stderr) = slot.gather()
            for file in self.copy_back_files:
                if slot.get_file(slot.work_root + '/' + work.filename + '-' + str(
                        work.quality) + file, runs_dst_dir + '/' + work.runid + '/' + work.set + '/') != 0:
                    rd_print(self.log, 'Failed to copy back ' +
                             work.filename +
                             '-' +
                             str(work.quality) +
                             file +
    ...
    def write_results(self):
        ...
        for line in lines:
            if new_line.split()[0] == line.split()[0]:
                rd_print(
                    self.log,
                    'Data already exists in out file, not writing',
                    self.get_name())
                return
        ...
        xml_filename = (runs_dst_dir +
                        ...
                        '-' +
                        str(self.quality) +
                        '-libvmaf.xml').encode('utf-8')
        with open(xml_filename, 'w') as f:
            f.write(self.vmaf_xml)

    def get_name(self):
        return self.filename + ' with quality ' + \
            str(self.quality) + ' for run ' + self.runid
...
class ABWork(Work):
    ...
    def execute(self):
        ...
        try:
            slot.setup(self.codec, self.bindir)
            slot.start_shell(slot.work_root +
                             '/rd_tool/ab_meta_compare.sh ' +
                             shellquote(str(self.bpp)) +
                             ' ' +
                             shellquote(self.runid) +
                             ' ' +
                             work.set +
                             ' ' +
                             shellquote(input_path) +
                             ' ' +
                             shellquote(self.codec))
            (stdout, stderr) = slot.gather()
            ...
            remote_file = slot.work_root + '/runs/' + \
                middle + '/' + shellquote(filename)
            ...
            rd_print(self.log, 'Encoding and copying', filename,
                     'at bpp', str(self.bpp), 'failed')
            rd_print(self.log, 'stdout:')
            rd_print(self.log, stdout.decode('utf-8'))
            rd_print(self.log, 'stderr:')
            rd_print(self.log, stderr.decode('utf-8'))
