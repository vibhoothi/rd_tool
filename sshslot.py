...
class Machine:
    def __init__(self, host, user='ec2-user', cores=18,
                 work_root='/home/ec2-user', port=22, media_path='/mnt/media'):
        self.host = host
        self.user = user
        self.cores = cores
    ...
    def rsync(self, local, remote):
        return subprocess.call(['rsync', '-r', '-e', "ssh -i " + ssh_privkey_file +
                               " -o StrictHostKeyChecking=no -p " + str(self.port), local, self.user + '@' + self.host + ':' + remote])
...
class Slot:
    ...
    def setup(self, codec, bindir):
        ...
        try:
            ...
            rd_print(
                self.log,
                'Couldn\'t connect to machine ' +
                self.machine.host)
            raise RuntimeError(
                'This is a bug with AWCY. Likely this machine has gone unreachable.')
        if self.machine.rsync('./', self.work_root + '/rd_tool/') != 0:
            rd_print(self.log, 'Couldn\'t set up machine ' + self.machine.host)
            raise RuntimeError(
                'Couldn\'t copy tools to machine (out of disk space?)')
        time.sleep(1)
        ...
        for binary in binaries[codec]:
            ...
            if self.machine.rsync(
                    bindir + '/' + binary, self.work_root + '/' + codec + '/' + binary) != 0:
                rd_print(
                    self.log,
                    'Couldn\'t upload codec binary ' +
                    binary +
                    'to ' +
                    self.machine.host)
    ...
    def get_file(self, remote, local):
        return subprocess.call(['rsync', '-e', "ssh -i " + ssh_privkey_file + " -o StrictHostKeyChecking=no -p " + str(
            self.machine.port), self.machine.user + '@' + self.machine.host + ':' + shellquote(remote), local])
